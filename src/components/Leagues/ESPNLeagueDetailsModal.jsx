...
const ESPNLeagueDetailsModal = ({ league, onClose, onGroupCreated }) => {
  const { currentUser } = useAuth();
  const [creating, setCreating] = useState(false);
  const [error, setError] = useState('');
  const [espnLogo, setEspnLogo] = useState(null);
  const [isConverted, setIsConverted] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    const loadESPNLogo = async () => {
      try {
        const logoUrl = await PlatformImageService.getImageUrl('espnFantasy.png');
        setEspnLogo(logoUrl);
      } catch (error) {
        console.error('Error loading ESPN logo:', error);
      }
    };
    loadESPNLogo();
  }, []);
  ...
  useEffect(() => {
    const checkExistingGroup = async () => {
      ...
      try {
        const exists = await groupService.checkGroupExists(league.id.toString(), 'espn');
        if (exists) {
          setIsConverted(true);
          if (onGroupCreated) {
            onGroupCreated();
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      const leagueInfo = {
        ...
        seasonYear: 2023, // TODO: Get actual season year from ESPN
...
export default ESPNLeagueDetailsModal;
