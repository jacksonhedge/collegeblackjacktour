...
export class OAuthService {
  ...
  private isValidRedirectUri(clientRedirectUris: string[], redirectUri: string): boolean {
  ...
  async getUserInfo(userId: string, scope: string[]): Promise<any> {
    const userInfo: any = {
  ...
  private async createIdToken(userId: string, clientId: string, scope: string[]): Promise<string> {
  ...
  async hasUserConsented(userId: string, clientId: string, scope: string[]): Promise<boolean> {
  ...
  async updateClientStatistics(clientId: string, type: 'authorization' | 'token'): Promise<void> {
